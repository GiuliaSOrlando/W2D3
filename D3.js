...
while(j < starWarsCharacters.length){
  crewMass += Number(starWarsCharacters[j].mass)
  j++
}
...
switch (true) {
	...
	case shipMass >= 900 && shipMass <= 1000:
		console.log('Critical Load: Over 900')
		break;

  case shipMass >= 700 && shipMass <= 900:
...
for(let i = 0; i < characters.length; i++){
  for(let j = 0; j < femaleCharacters.length; j++){
      if(characters[i] == femaleCharacters[j]){
          index.push(characters[i]);
          characters.splice(i, 1);
          femaleCharacters.splice(j, 1);
          break;
      }
  }
}

console.log(characters, index, femaleCharacters)


let index

for(i=0; i<characters.length; i++){
  if(characters.name === femaleCharacters.name) {
    characters.name.filter(i, 1)
  }
}
...
let item = items[Math.floor(Math.random()*starWarsCharacters.length)];
